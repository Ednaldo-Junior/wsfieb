...
public class AcordoController {
	...
	public static List<STRACORDOSVELOZ> consultaAcordos(String filial) throws SQLException {
		...
		String query = " SELECT  NUMACO,DTCACO,TOTACO,TPGPAR,VLRPAR,DTVPAR,SUBSTRING(ADMCLI from 1 for 4),ADMCLI FROM EJCBACO";
		query += " inner join ejcbcli on CRECLI = CREACO AND CONCLI = CONACO AND CRTCLI = CRTACO";
		query += " left join ejcbpar on CREPAR = CREACO AND NACPAR = NUMACO";
		query += " WHERE CREACO = 53 AND DTCACO = '04/16/2021'";
		query += " AND SUBSTRING(ADMCLI from 1 for 4) = '" + filial+"'";
		...
		while (result.next()) {
			...
			acordo.setZ06_FILIAL(result.getString("ADMCLI"));
	...
	public static List<STRTITACORDVELOZ> consultaTitulos(String filial) throws SQLException {
		...
		String query = "SELECT ";
		query += " CPFTIT,ACOTIT,NUMTIT,BCOTIT, AMOCLI,'S',PROCLI,ADMCLI  FROM EJCBACO";
		...
		while (result.next()) {
			...
			titulo.setE1_STATUS("4"); // todos os acordo devolve como 4
